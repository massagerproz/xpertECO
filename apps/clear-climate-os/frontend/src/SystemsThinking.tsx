          ...
          <div className="mb-8">
            ...
            <div className="space-y-4">
              {pendingVariables.map((v) => (
                <div key={v._id} className="p-4 bg-amber-50 border border-amber-100 rounded-lg flex justify-between items-start">
                  <div>
                  ...
                  <div className="flex gap-2 ml-4">
                    <button onClick={() => updateSystemVariableStatus({ id: v._id, status: "approved" })} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-md">
                      <Check className="w-4 h-4" />
                    </button>
                    <button onClick={() => updateSystemVariableStatus({ id: v._id, status: "rejected" })} className="p-1.5 text-rose-600 hover:bg-rose-50 rounded-md">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                </div>
              ...
              {pendingLinks.map((l) => (
                <div key={l._id} className="p-4 bg-amber-50 border border-amber-100 rounded-lg flex justify-between items-start">
                  <div>
                    <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 mb-2">
                  ...
                  <div className="flex gap-2 ml-4">
                    <button onClick={() => updateSystemLinkStatus({ id: l._id, status: "approved" })} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-md">
                      <Check className="w-4 h-4" />
                    </button>
                    <button onClick={() => updateSystemLinkStatus({ id: l._id, status: "rejected" })} className="p-1.5 text-rose-600 hover:bg-rose-50 rounded-md">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="grid gap-2 grid-cols-1 md:grid-cols-2">
                  {approvedVariables.map((v) => (
                    <div key={v._id} className="p-3 border border-slate-200 rounded-lg bg-white">
                      <p className="font-medium text-sm text-slate-800">{v.name}</p>
              ...
              {approvedLinks.length > 0 && (
                <div>
                  ...
                  <div className="grid gap-2">
                    {approvedLinks.map((l) => (
                      <div key={l._id} className="p-3 border border-slate-200 rounded-lg bg-white flex items-center gap-4">
